...
public class Solution {
    ...
    //@ requires(*The string parameter `a` and `b` consist of only English lowercase letters.*);
    //@ ensures(*If the string parameter `a` and `b` are not equal, the integer result is greater than or equal to 1 and is less than or equal to the length of the longer string parameter.*);
    //@ ensures(*If the string parameter `a` and `b` are equal, the integer result is equal to -1.*);
