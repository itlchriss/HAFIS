...
//@ ensures((\result <= gas.length - 1) && (\result >= -1));
...
//@ ensures((\result >= 0) ==> (\result < gas.length));
//@ ensures(((gas[0] == 2 && gas[1] == 3 && gas[2] == 4 && gas.length == 3) && (cost[0] == 3 && cost[1] == 4 && cost[2] == 3 && cost.length == 3)) ==> (\result == -1));
//@ ensures(((\sum int i; 0 <= i < gas.length; gas[i]) < (\sum int i; 0 <= i < cost.length; cost[i])) ==> (\result == -1));
