...
//@ requires((\forall int i; 0 <= i < cost.length; cost[i] >= 0) && (\forall int i; 0 <= i < cost.length; cost[i] <= 10000));
//@ requires(cost.length == gas.length);
//@ requires((\forall int i; 0 <= i < gas.length; gas[i] >= 0) && (\forall int i; 0 <= i < gas.length; gas[i] <= 10000));
//@ ensures((\result >= -1) && (\result < gas.length));
//@ ensures((\result == -1) ==> ((\sum int i; 0 <= i < gas.length; gas[i]) < (\sum int i; 0 <= i < cost.length; cost[i])));
