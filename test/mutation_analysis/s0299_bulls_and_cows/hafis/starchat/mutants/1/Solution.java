...
//@ requires((\forall int i; 0 <= i < secret.length(); Character.isDigit(secret.charAt(i))) && (\forall int i; 0 <= i < guess.length(); Character.isDigit(guess.charAt(i))));
//@ requires(((secret.length() <= 1000) && (secret.length() >= 1)) && ((guess.length() <= 1000) && (guess.length() >= 1)));
