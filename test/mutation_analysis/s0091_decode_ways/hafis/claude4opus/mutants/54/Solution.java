...
//@ requires(\forall int i; 0 <= i < s.length(); Character.isDigit(s.charAt(i)));
//@ ensures((\result <= 2147483647) && (\result >= 0));
//@ ensures((s.equals("226")) ==> (\result == 3));
//@ ensures((s.equals("12")) ==> (\result == 2));
//@ ensures(("00" != null && s != null && "00".length() <= s.length() && (\exists int i; 0 <= i && i <= s.length() - "00".length(); (\forall int j; 0 <= j && j < "00".length(); s.charAt(i + j) == "00".charAt(j)))) ==> (\result == 0));
