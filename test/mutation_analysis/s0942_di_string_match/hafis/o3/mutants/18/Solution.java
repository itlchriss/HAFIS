...
//@ requires((\forall int i; 0 <= i < s.length(); 'I' == s.charAt(i)) && (\forall int i; 0 <= i < s.length(); 'D' == s.charAt(i)));
//@ ensures(\result.length == s.length() + 1);
//@ ensures((\forall int i; 0 <= i < \result.length; (\forall int j; 0 <= j < \result.length && j != i; \result[j] != \result[i])) && ((\forall int i; 0 <= i < \result.length; \result[i] >= 0) && (\forall int i; 0 <= i < \result.length; \result[i] <= s.length())));
...
//@ ensures((s.equals("IDID")) ==> (\result[0] == 0 && \result[1] == 4 && \result[2] == 1 && \result[3] == 3 && \result[4] == 2 && \result.length == 5));
    public int[] diStringMatch(String s) {
        ...
        for (int i = s.length() - 1; true && max > 0; i--) {
