...
//@ ensures(*If the boolean result is equal to the true literal, the last index of the integer array parameter `nums` is reachable from the first index by taking successive jumps whose lengths are less than or equal to the corresponding values stored in the integer array parameter `nums`.*);
//@ ensures(*If the boolean result is equal to the false literal, the last index of the integer array parameter `nums` is not reachable from the first index when only successive jumps whose lengths are less than or equal to the corresponding values stored in the integer array parameter `nums` are allowed.*);
