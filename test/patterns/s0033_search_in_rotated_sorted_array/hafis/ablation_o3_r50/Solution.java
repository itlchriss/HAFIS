...
//@ requires(*The integer array parameter `nums` is in ascending order and may be rotated around one pivot index.*);
//@ requires(*The integer parameter `target` is greater than or equal to -10000 and is less than or equal to 10000.*);
//@ ensures(*If the integer parameter `target` is present in the integer array parameter `nums`, the integer result is equal to the index of the integer parameter `target` in the integer array parameter `nums`.*);
//@ ensures(*If the integer parameter `target` is not present in the integer array parameter `nums`, the integer result is equal to -1.*);
//@ ensures(*If the integer result is different from -1, the integer result is greater than or equal to 0 and is less than the length of the integer array parameter `nums`.*);
