...
//@ requires(*The integer array parameter `nums` is sorted in ascending order or is a rotated ascending array.*);
//@ requires(*The integer parameter `target` is less than or equal to 10000 and is greater than or equal to -10000.*);
//@ ensures(*The integer result is less than the length of the integer array parameter `nums` and is greater than or equal to -1.*);
//@ ensures(*If the integer result is equal to -1, the integer parameter `target` is not an element of the integer array parameter `nums`.*);
//@ ensures(*If the integer result is greater than or equal to 0, the value at the integer index equal to the integer result in the integer array parameter `nums` is equal to the integer parameter `target`.*);
