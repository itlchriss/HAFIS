...
//@ requires(*All the values in the integer array parameter `digits` are greater than or equal to 0 and are less than or equal to 9.*);
//@ requires(*The first value of the integer array parameter `digits` is not equal to 0 unless the length of the integer array parameter `digits` is equal to 1.*);
//@ ensures(*The length of the integer array result is equal to the length of the integer array parameter `digits` or is equal to the length of the integer array parameter `digits` plus 1.*);
...
//@ ensures(*All the values in the integer array result are greater than or equal to 0 and are less than or equal to 9.*);
//@ ensures(*The first value of the integer array result is equal to 1 if and only if all the values in the integer array parameter `digits` are equal to 9.*);
