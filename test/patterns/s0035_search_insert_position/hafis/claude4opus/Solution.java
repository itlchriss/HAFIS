...
//@ ensures(*If the integer parameter `target` exists in the integer array parameter `nums`, the integer result is the index of the integer parameter `target` in the integer array parameter `nums`.*);
//@ ensures(*If the integer parameter `target` does not exist in the integer array parameter `nums` and is less than all values in the integer array parameter `nums`, the integer result is equal to 0.*);
//@ ensures(*If the integer parameter `target` does not exist in the integer array parameter `nums` and is greater than all values in the integer array parameter `nums`, the integer result is equal to the length of the integer array parameter `nums`.*);
