...
//@ requires(*All values in the integer array parameters `nums1`and`nums2` are greater than or equal to 0 and are less than or equal to 10000*);
//@ requires(*All values in the integer array parameters `nums1`and`nums2` are unique*);
...
//@ ensures(*For each index i within the bounds of the integer array parameter `nums1` if a value greater than the value at index i in the integer array parameter `nums1` occurs to the right of the matching value in the integer array parameter `nums2` the value at index i in the integer array result is equal to the first such greater value found in the integer array parameter `nums2`*);
//@ ensures(*For each index i within the bounds of the integer array parameter `nums1` if no value greater than the value at index i in the integer array parameter `nums1` occurs to the right of the matching value in the integer array parameter `nums2` the value at index i in the integer array result is equal to -1*);
