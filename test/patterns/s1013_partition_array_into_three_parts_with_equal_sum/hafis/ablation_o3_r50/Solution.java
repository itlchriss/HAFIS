...
//@ ensures(*If the boolean result is equal to the true literal, the integer array parameter `arr` can be partitioned into three consecutive non empty segments whose sums are equal.*);
//@ ensures(*If the boolean result is equal to the false literal, the integer array parameter `arr` cannot be partitioned into three consecutive non empty segments whose sums are equal.*);
