...
//@ ensures(*If boolean result is equal to the true literal, the integer array parameter `arr` can be split into three contiguous non empty sections such that the sum of the first section is equal to the sum of the second section and is equal to the sum of the third section.*);
//@ ensures(*If boolean result is equal to the false literal, the integer array parameter `arr` cannot be split into three contiguous non empty sections having equal sums.*);
