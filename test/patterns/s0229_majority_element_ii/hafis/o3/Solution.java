...
//@ ensures(*The length of the list result is less than or equal to 2.*);
//@ ensures(*All integer values in the list result are unique.*);
//@ ensures(*Each integer value in the list result appears in the integer array parameter `nums` more times than the quotient between the length of the integer array parameter `nums` and 3.*);
//@ ensures(*If an integer value is not contained in the list result, the number of its occurrences in the integer array parameter `nums` is less than or equal to the quotient between the length of the integer array parameter `nums` and 3.*);
