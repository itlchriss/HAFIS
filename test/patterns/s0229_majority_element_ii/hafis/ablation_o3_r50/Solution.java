...
//@ ensures(*For each integer value in the list result, the occurrence count of that integer value in the integer array parameter `nums` is greater than the length of the integer array parameter `nums` divided by 3.*);
//@ ensures(*All integer values in the list result are contained in the integer array parameter `nums`.*);
//@ ensures(*The size of the list result is less than or equal to 2.*);
