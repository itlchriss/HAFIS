...
//@ requires(*The string parameter `pattern` consists of only lowercase English letters or the characters ? or *.*);
//@ ensures(*If the boolean result is equal to the true literal, the string parameter `pattern` matches the entire string parameter `inputString` under the rules that the character ? matches any single character and the character * matches any sequence of characters including the empty sequence.*);
//@ ensures(*If the boolean result is equal to the false literal, the string parameter `pattern` does not match the entire string parameter `inputString` under the rules that the character ? matches any single character and the character * matches any sequence of characters including the empty sequence.*);
