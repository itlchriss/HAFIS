...
char *dafny_contain_construct(char *d1, char *d2, struct datatype *dt1, struct datatype *dt2) {
    ...
    if (si == NULL) {
        printf("dafny_contain_construct SI not found\n");
        sinotfound_error(d2);
    }
    ...
    while (_t != NULL) {
        ...
        if (_t[0] == '\'' && _t[strlen(_t) - 1] == '\'') {
            ...
            if (siq == NULL || siq->count == 0) {
                printf("dafny.c dafny_contain_only_string_chararray\n");
                sinotfound_error(_t);
            }
            struct si *si = searchqueue(siq, _t, __match_si_with_datatype_char_only__);
            if (si == NULL) {
                printf("dafny.c dafny_contain_only_string_chararray no noun type TSI template\n");
                sinotfound_error(_t);
            }
...
/*
    ...
    The dafny_ops struct is defined in backend_dafny.c.
