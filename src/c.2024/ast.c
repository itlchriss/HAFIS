...
/*
    ...
    After synthesis, many Quantifier and Connective nodes have all their
    children consumed. This function marks such nodes as consumed too,
    cascading bottom-up so the IR builder sees a clean tree.
    
    Rules:
    - Quantifier with 0 active children → mark as consumed
    - Connective with 0 active children → mark as consumed
    - The root pointer is updated if the root itself is consumed
*/
static void mark_empty_nodes_consumed(struct astnode *node) {
    if (!node) return;
    
    /* First, recurse into all children (bottom-up traversal) */
    struct astnodelist *child = node->children;
    while ((child = child->next) != NULL) {
        if (child->node && child->node->status == AST_ACTIVE) {
            mark_empty_nodes_consumed(child->node);
        }
    }
    
    /* Skip if already consumed */
    if (node->status == AST_CONSUMED) return;
    
    /* Only process Quantifier and Connective nodes */
    if (node->type == Quantifier || node->type == Connective) {
        int active = countastchildren_active(node);
        if (active == 0) {
            node->status = AST_CONSUMED;
        }
    }
...
void ast_simplify_after_synthesis(struct astnode **root_ptr) {
    ...
    mark_empty_nodes_consumed(*root_ptr);
    /* If root was consumed, find the new logical root */
    if ((*root_ptr)->status == AST_CONSUMED) {
        *root_ptr = find_logical_root(*root_ptr);
    }
}
