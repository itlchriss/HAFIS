...
method productExceptSelf(nums: array<int>) returns (result: array<int>)
{
    ...
    // ensures(*For every non-negative integer `i` such that `i` is less than the length of the integer array parameter `nums`, the value at index `i` of the integer array result is equal to the product of all values in the integer array parameter `nums` except the value at index `i` of the integer array parameter `nums`.*);
