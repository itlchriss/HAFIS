...
method isPowerOfTwo(n: int) returns (result: bool)
{
    ...
    // ensures(*The boolean result is equal to the true literal if and only if there exists a non-negative integer `x` such that the integer parameter `n` is equal to 2 raised to the power of the non-negative integer `x`.*);
    if n <= 0 {
        result := false;
        return;
    }
