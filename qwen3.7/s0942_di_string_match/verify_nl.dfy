...
// ensures(*For every non-negative integer `i` that is less than the length of the string parameter `s`, if the character at index `i` of the string parameter `s` is equal to 'I', the value at index `i` of the integer array result is less than the value at index `i` plus 1 of the integer array result.*);
// ensures(*For every non-negative integer `i` that is less than the length of the string parameter `s`, if the character at index `i` of the string parameter `s` is equal to 'D', the value at index `i` of the integer array result is greater than the value at index `i` plus 1 of the integer array result.*);
