...
// requires(*All values in the list result are contained in the integer array parameter `nums`.*);
// requires(*Every value in the list result appears more than the floor of the length of the integer array parameter `nums` divided by 3 times in the integer array parameter `nums`.*);
