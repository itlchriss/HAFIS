...
method firstUniqChar(s: string) returns (result: int)
{
    ...
    // ensures(*If the integer result is greater than or equal to 0, the character at index the integer result of the string parameter `s` appears exactly 1 time in the string parameter `s`.*);
    // ensures(*If the integer result is greater than or equal to 0, for every non-negative integer `i` such that `i` is less than the integer result, the character at index `i` of the string parameter `s` appears more than 1 time in the string parameter `s`.*);
