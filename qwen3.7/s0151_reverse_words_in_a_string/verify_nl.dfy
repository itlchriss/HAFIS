...
method reverseWords(s: string) returns (result: string)
{
    ...
    // requires(*The string parameter `s` consists of only English letters and digits and the character ` `.*);
    ...
    // ensures(*The string result contains the same words as the string parameter `s` in reverse order.*);
    // ensures(*The string result does not contain leading spaces or trailing spaces.*);
