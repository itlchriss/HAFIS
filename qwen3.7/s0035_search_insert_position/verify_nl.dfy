...
// ensures(*If the integer result is less than the length of the integer array parameter `nums`, for every non-negative integer `k` the integer result is less than the integer result, the value at index `k` of the integer array parameter `nums` is strictly less than the integer parameter `target`.*);
// ensures(*If the integer result is greater than 0 and is less than the length of the integer array parameter `nums`, for every non-negative integer `k` the integer result is greater than or equal to the integer result and is less than the length of the integer array parameter `nums`, the value at index `k` of the integer array parameter `nums` is greater than or equal to the integer parameter `target`.*);
